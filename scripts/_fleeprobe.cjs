/* 打包真实 RunawayButtons 到无头浏览器。产出 out/renderer/_fleeprobe.html
   用法：node scripts/_fleeprobe.cjs */
const fs = require('fs')
...
const entry = path.join(ROOT, 'scripts/_fleeprobe_entry.tsx')
const jsOut = path.join(OUT, '_fleeprobe.js')
...
<body>
<div id="host"></div>
<script src="./_fleeprobe.js"></script>
</body>
</html>
`

fs.writeFileSync(path.join(OUT, '_fleeprobe.html'), html)
console.log('css  ->', cssFile)
console.log('js   ->', path.basename(jsOut), Math.round(fs.statSync(jsOut).size / 1024) + 'kB')
console.log('html ->', 'out/renderer/_fleeprobe.html')
