...
var require_zip_bin = __commonJS({
  "node_modules/7zip-bin/index.js"(exports2) {
    "use strict";
    ...
    function getPath() {
      ...
      if (process.platform === "darwin") {
        return path6.join(__dirname, "mac", process.arch, "7za");
      } else if (process.platform === "win32") {
        return path6.join(__dirname, "win", process.arch, "7za.exe");
      } else {
        return path6.join(__dirname, "linux", process.arch, "7za");
      }
    }
    exports2.path7za = getPath();
    exports2.path7x = path6.join(__dirname, "7x.sh");
  }
});

// src/main/services/CustomLevelService.ts
var import_fs2 = __toESM(require("fs"));
var import_path3 = __toESM(require("path"));

// src/main/services/LevelStudioInstaller.ts
...
var TEXTURE_PACKS_RELATIVE = ["BALDI_Data", "StreamingAssets", "Texture Packs"];
var TEXTURE_PACK_MANIFEST = "pack.json";
var PROTECTED_TEXTURE_PACK_FOLDERS = /* @__PURE__ */ new Set(["core"]);
var THEME_DEFAULT = "dark";
var FONT_DEFAULT = "Comic Sans MS";
function texturePacksDir(gameRoot) {
  return import_path.default.join(gameRoot, ...TEXTURE_PACKS_RELATIVE);
}
...
// src/main/services/LevelStudioInstaller.ts
var README_MAX_BYTES = 256 * 1024;
function levelStudioPlayablesPath() {
...
// src/main/services/CustomLevelParser.ts
var import_fs = __toESM(require("fs"));
var PNG_MAGIC = Buffer.from([137, 80, 78, 71, 13, 10, 26, 10]);
var GUID_RUN = /^[\w-]{8}-[\w-]{4}-[\w-]{4}-[\w-]{4}-[\w-]{12}$/i;
function isPrintableAscii(b) {
  return b >= 32 && b <= 126;
}
function collectStrings(buf, end) {
  const out = [];
  let i = 0;
...
function readCustomLevel(pbplPath, size) {
  let buf;
  let statSize = size;
  try {
    buf = import_fs.default.readFileSync(pbplPath);
    if (statSize <= 0) statSize = buf.length;
  } catch {
    return null;
  }
  ...
  let author = "";
  let type = "";
  if (pool.length > 0) name = pool[0];
  if (pool.length > 1) author = pool[1];
...
function listCustomLevels() {
  ...
  let entries;
  try {
    entries = import_fs2.default.readdirSync(playables, { withFileTypes: true });
  } catch {
    return out;
  }
  for (const entry of entries) {
    ...
    if (name.endsWith(DISABLED_SUFFIX)) {
      if (!isPbpl(stripDisabled(name))) continue;
    } else if (!isPbpl(name)) {
      continue;
    }
    const base = stripDisabled(name);
    const enabled = !name.endsWith(DISABLED_SUFFIX);
    const full = import_path3.default.join(playables, name);
    let size = 0;
    try {
      size = import_fs2.default.statSync(full).size;
    } catch {
    }
...
// src/main/services/TexturePackService.ts
var import_fs3 = __toESM(require("fs"));
var import_path4 = __toESM(require("path"));

// src/main/services/ModArchiveExtractor.ts
var import_electron2 = __toESM(require_stub_electron());
var import_adm_zip = __toESM(require_adm_zip());
var import_zip_bin = __toESM(require_zip_bin());

// src/main/services/TexturePackService.ts
function readPackJson(dir) {
  const p = import_path4.default.join(dir, TEXTURE_PACK_MANIFEST);
  if (!import_fs3.default.existsSync(p)) return null;
  try {
    const raw = JSON.parse(import_fs3.default.readFileSync(p, "utf8"));
    return {
      name: typeof raw["Name"] === "string" ? raw["Name"] : void 0,
      author: typeof raw["Author"] === "string" ? raw["Author"] : void 0,
      version: raw["Version"] !== void 0 ? String(raw["Version"]) : void 0,
...
function isProtectedTexturePack(folderName) {
  return PROTECTED_TEXTURE_PACK_FOLDERS.has(String(folderName).toLowerCase());
}
function listTexturePacks(gameRoot) {
...
function main() {
  ...
  const tpDir = path5.join(gameRoot, "BALDI_Data/StreamingAssets/Texture Packs");
  console.log("--- Texture Packs \u76EE\u5F55\u5B9E\u9645\u5185\u5BB9 ---");
  try {
    for (const f of fs4.readdirSync(tpDir)) console.log("  ", f);
  } catch (e) {
    console.log("  \u8BFB\u53D6\u5931\u8D25:", e instanceof Error ? e.message : String(e));
  }
}
