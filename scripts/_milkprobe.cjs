/* 打包真实 MilkPage 到无头浏览器。产出 out/renderer/_milkprobe.html
   用法：node scripts/_milkprobe.cjs */
const fs = require('fs')
...
const entry = path.join(ROOT, 'scripts/_milkprobe_entry.tsx')
const jsOut = path.join(OUT, '_milkprobe.js')
...
<body>
<div id="host"></div>
<script src="./_milkprobe.js"></script>
</body>
</html>
`

fs.writeFileSync(path.join(OUT, '_milkprobe.html'), html)
console.log('css  ->', cssFile)
console.log('js   ->', path.basename(jsOut), Math.round(fs.statSync(jsOut).size / 1024) + 'kB')
console.log('html ->', 'out/renderer/_milkprobe.html')
