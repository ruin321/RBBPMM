/* 打包真实 GlitchPage 到无头浏览器。产出 out/renderer/_glitchprobe.html
   用法：node scripts/_glitchprobe.cjs */
const fs = require('fs')
...
const entry = path.join(ROOT, 'scripts/_glitchprobe_entry.tsx')
const jsOut = path.join(OUT, '_glitchprobe.js')
...
<body>
<div id="host"></div>
<script src="./_glitchprobe.js"></script>
</body>
</html>
`

fs.writeFileSync(path.join(OUT, '_glitchprobe.html'), html)
console.log('css  ->', cssFile)
console.log('js   ->', path.basename(jsOut), Math.round(fs.statSync(jsOut).size / 1024) + 'kB')
console.log('html ->', 'out/renderer/_glitchprobe.html')
