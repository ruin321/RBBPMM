/* 打包真实 JumpScare 到无头浏览器。产出 out/renderer/_scareprobe.html
   用法：node scripts/_scareprobe.cjs */
const fs = require('fs')
...
const entry = path.join(ROOT, 'scripts/_scareprobe_entry.tsx')
const jsOut = path.join(OUT, '_scareprobe.js')
...
<body>
<div id="host"></div>
<script src="./_scareprobe.js"></script>
</body>
</html>
`

fs.writeFileSync(path.join(OUT, '_scareprobe.html'), html)
console.log('css  ->', cssFile)
console.log('js   ->', path.basename(jsOut), Math.round(fs.statSync(jsOut).size / 1024) + 'kB')
console.log('html ->', 'out/renderer/_scareprobe.html')
