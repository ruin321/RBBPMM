...
export function TexturePacksPage({ env, dropPath, onDropConsumed }: Props): React.JSX.Element {
    ...
    return (<div className="mx-auto w-full max-w-3xl space-y-6">
      <PageHeader icon={<Palette className="h-6 w-6"/>} title={t('textures.title')} desc={t('textures.countInstalled', { n: packs.length })}>
      ...
      {readmes.length > 0 && (<ReadmeDialog kind="texture" readmes={readmes} installDir={readmeDir} onOpenChange={(open) => {
                if (!open)
                    setReadmes([]);
            }}/>)}
    </div>);
