...
export function ToolboxPage({ onSetup, env }: Props): React.JSX.Element {
    ...
    const gib = (n: number): string => {
        let s = '';
        for (let i = 0; i < n; i++)
            s += GIB[Math.floor(Math.random() * GIB.length)];
        return s;
    };
    ...
    const openUrlEasterEgg = (): void => {
        ...
        void window.api.ui.openExternal(isZh
    ...
    return (<div className="mx-auto w-full max-w-3xl space-y-6">
      ...
      <Card className="border-destructive/40">
        ...
        <CardContent className="flex flex-wrap gap-3">
          <WithTooltip title={t('toolbox.easterDoNot')}>
            <Button variant="destructive" onClick={() => prankEffect()}>
      ...
      {overlay === 'gibberish' && (<div className="prank-veil fixed inset-0 z-50 cursor-pointer overflow-hidden" onClick={() => setOverlay(null)}>
          <pre className="pointer-events-none select-none break-all whitespace-pre-wrap p-2 text-[10px] leading-tight prank-gib-text">
            {gibText}
          </pre>
        </div>)}

      {overlay === 'crash' && (<div className="fixed inset-0 z-50 cursor-pointer bg-[#083bf5] px-10 py-8 text-white" onClick={() => setOverlay(null)}>
