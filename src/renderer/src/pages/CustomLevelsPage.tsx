...
export function CustomLevelsPage({ modInstalled }: {
    ...
    return (<div className="space-y-6">
      ...
      {!modInstalled ? (<div className="flex flex-col items-center justify-center gap-3 py-20 text-center">
        ...
        </div>) : levels.length === 0 ? (<div className="flex flex-col items-center justify-center gap-3 py-20 text-center">
          <MapIcon className="h-12 w-12 text-muted-foreground"/>
          ...
          {levels.map((lvl) => (<Card key={lvl.fileName} className={!lvl.enabled ? 'opacity-60' : ''}>
              <CardContent className="flex items-center gap-3 p-3">
                <div className="flex h-16 w-16 shrink-0 items-center justify-center overflow-hidden rounded-md bg-muted">
                  {lvl.thumbnail ? (<img src={lvl.thumbnail} alt={lvl.name} className="h-full w-full object-cover"/>) : (<FileQuestion className="h-6 w-6 text-muted-foreground"/>)}
                </div>
                ...
                <div className="flex shrink-0 flex-col items-end gap-2">
                  ...
                  <Button variant="ghost" size="icon" className="h-6 w-6 text-destructive" onClick={() => setDeleteTarget(lvl)}>
