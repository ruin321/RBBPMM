...
export function useMods(): {
    ...
    useEffect(() => {
        ...
        const off = window.api.app.onInstallProgress((p) => {
            if (mounted.current)
                setInstallState({ status: 'running', progress: p });
        });
        return () => {
    ...
    const startInstall = useCallback(async (archivePath: string): Promise<'ok' | 'confirm' | 'error'> => {
        setInstallState({
            status: 'running',
        ...
        if (out.mode === 'manifest' || out.mode === 'unmanaged') {
            ...
            if ((out.readmes?.length ?? 0) > 0)
                setReadmes(out.readmes);
            await refresh();
            return 'ok';
        }
    ...
    const toggle = useCallback(async (guid: string, activate: boolean): Promise<boolean> => {
    ...
    const uninstall = useCallback(async (guid: string): Promise<boolean> => {
    ...
    const updateMod = useCallback(async (guid: string): Promise<boolean> => {
