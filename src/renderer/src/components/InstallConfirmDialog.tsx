...
export function InstallConfirmDialog({ open, plan, busy, onOpenChange, onConfirm }: Props): React.JSX.Element {
    const { t } = useI18n();
    if (!plan)
        return <></>;
    return (<Dialog open={open} onOpenChange={(o) => !busy && onOpenChange(o)}>
      <DialogContent className="sm:max-w-md">
        ...
        <div className="space-y-4">
          {plan.modded.length === 0 && plan.plugins.length === 0 && (<p className="text-sm text-muted-foreground">{t('mods.confirmNone')}</p>)}

          {plan.modded.length > 0 && (<div>
              <div className="mb-1 text-sm font-semibold">{t('mods.confirmToModded')}</div>
              <ul className="space-y-1">
                {plan.modded.map((name) => (<li key={name} className="text-xs text-muted-foreground">
          ...
          {plan.plugins.length > 0 && (<div>
              <div className="mb-1 text-sm font-semibold">{t('mods.confirmToPlugins')}</div>
              <ul className="space-y-1">
                {plan.plugins.map((rel) => (<li key={rel} className="text-xs text-muted-foreground">
