...
export function ModListItem({ mod, updateInfo, updating, onUpdate, onToggle, onUninstall, onEditConfig, pinned, onSetPinned }: Props): React.JSX.Element {
    const { t } = useI18n();
    ...
    const uninstall = async (): Promise<void> => {
        ...
        setBusy(false);
        if (ok)
            toast.success(t('mods.uninstalled'), { description: mod.name });
        else
            toast.error(t('mods.failUninstall'), { description: mod.name });
    };
    return (<Card className="relative w-full">
      ...
      <CardContent className="flex items-center gap-4 p-4">
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-2">
            ...
            {!mod.supportsCurrentVersion && (<Badge variant="warning">{t('mods.notMatch')}</Badge>)}
          </div>
          <div className="mt-1 flex flex-wrap items-center gap-2 text-xs text-muted-foreground">
            <span>{mod.author}</span>
            <Badge variant="secondary">v{mod.version}</Badge>
          </div>
          {updateInfo && (<div className="mt-2 flex flex-wrap items-center gap-2 rounded-md bg-primary/10 px-2.5 py-1.5 text-xs">
              ...
              <Button size="sm" variant="default" disabled={updating} onClick={onUpdate} className="h-7 gap-1 px-2.5">
        ...
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-2">
            <Switch checked={pending !== null ? pending : mod.activated} disabled={busy} onCheckedChange={(c) => void toggle(c)}/>
          </div>
          <Dialog>
            <DropdownMenu>
            ...
            (mod.guid.startsWith('legacy:')
                ...
                {mod.dllFile && (mod.loose ? (<DropdownMenuItem onClick={() => void window.api.ui.revealFile(mod.dllFile!)}>
                      <Eye className="mr-2 h-4 w-4"/>
                      {t('list.revealInExplorer')}
                    </DropdownMenuItem>) : (mod.dllDirectory && (<DropdownMenuItem onClick={() => void window.api.ui.openFolder(mod.dllDirectory!)}>
                        <FolderPlus className="mr-2 h-4 w-4"/>
                        {t('list.openDllFolder')}
                      </DropdownMenuItem>)))}
                {onEditConfig && (<DropdownMenuItem onClick={() => {
                onEditConfig(mod.configFile, mod.identifyName || '');
            }}>
                    <ClipboardList className="mr-2 h-4 w-4"/>
                    {t('list.editConfig')}
                  </DropdownMenuItem>)}
                {onSetPinned && (<DropdownMenuItem onClick={() => onSetPinned(!pinned)}>
