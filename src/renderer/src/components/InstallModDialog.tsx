...
export function InstallModDialog({ open, onOpenChange, progress, status, modName, errorMessage, onCancel }: Props): React.JSX.Element {
    const { t } = useI18n();
    ...
    return (<Dialog open={open} onOpenChange={handleOpenChange}>
      <DialogContent className="sm:max-w-md" onInteractOutside={(e) => {
        ...
        {status === 'running' && progress && (<div className="space-y-2">
        ...
        <DialogFooter>
          {status === 'running' ? (<Button variant="outline" onClick={() => handleOpenChange(false)} disabled={busy}>
              {t('dialog.cancel')}
            </Button>) : (<Button onClick={() => onOpenChange(false)}>{t('dialog.close')}</Button>)}
        </DialogFooter>
      </DialogContent>
    </Dialog>);
}
