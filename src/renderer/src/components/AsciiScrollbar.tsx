...
export function AsciiScrollbar({ selector = 'main.flex-1.overflow-y-auto' }: Props): React.JSX.Element | null {
  ...
  if (!metrics || rows < MIN_ROWS) return null
  ...
  return (
    <div
      aria-hidden="true"
      className="ascii-scrollbar pointer-events-none fixed z-30 select-none text-center"
      style={{ left: metrics.left, top: metrics.top + offset }}
    >
      {cells}
    </div>
  )
