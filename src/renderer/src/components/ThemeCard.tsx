      ...
      <CardContent>
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
          {THEMES.map((th) => {
            ...
            return (<button key={th.id} type="button" onClick={() => onChange(th.id)} className={cn('flex items-center gap-3 rounded-lg border p-3 text-left transition', active
                    ? 'border-ring bg-accent/60 ring-1 ring-ring'
                    : 'border-input hover:bg-accent/40')}>
                <span className="h-8 w-8 shrink-0 rounded-full border border-black/10" style={{ background: `hsl(${swatch})` }}/>
