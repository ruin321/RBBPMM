...
export function RansomOverlay({ onUnlocked }: Props): React.JSX.Element {
  ...
  // 拖满 20 个文件 -> 直接进入 THANK YOU 阶段
  useEffect(() => {
    if (!unlocked || phase !== 'ransom') return
    setPhase('thankyou')
  }, [unlocked, phase])

  // THANK YOU 阶段播感谢音并稍后进入详情页
  useEffect(() => {
    ...
    return (
      ...
      >
        ...
        <div className="flex flex-col items-center gap-6">
          <h1
            className="text-center text-5xl font-black uppercase tracking-[0.3em] text-white"
            style={{ textShadow: '4px 4px 0 #000, -2px 0 0 #000' }}
          >
            THANK
  ...
  return (
    <div
      ...
      style={{
        background:
          'radial-gradient(ellipse at center, #7a0f0f 0%, #360000 30%, #100000 60%, #000 100%)'
      }}
    >
      {/* 全屏红色噪点/扫描线 */}
      <div
        ...
        style={{
          backgroundImage:
            "repeating-linear-gradient(0deg, rgba(255,0,0,0.5) 0px, rgba(255,0,0,0.5) 1px, transparent 1px, transparent 2px), repeating-linear-gradient(90deg, rgba(0,0,0,0.3) 0px, rgba(0,0,0,0.3) 2px, transparent 2px, transparent 4px)"
        }}
      />
      ...
      <div className="absolute inset-x-0 top-8 flex flex-col items-center gap-2">
        ...
        <h1
          className="text-center text-2xl font-black uppercase tracking-[0.08em] text-black sm:text-3xl"
          style={{ textShadow: '1px 1px 0 #fff, -1px -1px 0 #fff', WebkitTextStroke: '1px rgba(255,255,255,0.35)' }}
        >
          YOUR MODS
          <br />
        ...
        <div className="relative bg-gradient-to-b from-red-600 to-red-800 p-[5px] shadow-[0_0_50px_rgba(255,0,0,0.5)]">
          <div className="px-5 py-6 text-center">
            ...
            <div className="relative mx-auto mb-3 flex w-fit items-center justify-center">
              <span
                aria-hidden
                className="absolute -inset-2 -z-10 rounded-full opacity-60 blur-md"
                style={{ background: 'linear-gradient(90deg,#00f,transparent,#f0f)' }}
              />
            ...
            <div className="mx-auto mt-4 flex items-center justify-center gap-3">
              <button
                type="button"
                onClick={() => {}}
                className="flex items-center gap-1 border-2 border-yellow-400 bg-black px-4 py-1 font-black text-yellow-400"
              >
                {received}
      ...
      {dragging && (
        <div className="pointer-events-none absolute inset-3 z-10 rounded-lg border-4 border-dashed border-red-500/80 bg-red-500/10" />
      )}
    </div>
  )
}