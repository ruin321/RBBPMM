...
export function GameDirCard({ env, loading, onSelect, onSetup }: Props): React.JSX.Element {
    ...
    return (<Card className="w-full">
      ...
      <CardContent>
        {loading ? (<Skeleton className="h-6 w-2/3"/>) : env ? (<div className="space-y-3">
            ...
            <div className="flex flex-wrap items-center gap-2 text-sm">
              ...
              {onSetup && (<Button variant="outline" size="sm" onClick={() => onSetup()}>
                  <Wrench className="mr-1 h-4 w-4"/>
                  {t('settings.setup')}
                </Button>)}
            </div>
          </div>) : (<p className="text-sm text-muted-foreground">{t('dir.none')}</p>)}
      </CardContent>
    </Card>);
}
