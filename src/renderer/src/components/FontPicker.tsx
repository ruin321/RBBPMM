      ...
      <CardHeader className="flex flex-row items-start justify-between space-y-0">
        ...
        <Dialog open={open} onOpenChange={setOpen}>
          ...
          <DialogContent className="flex h-[28rem] max-w-md flex-col">
            ...
            <input autoFocus value={query} onChange={(e) => setQuery(e.target.value)} placeholder={t('font.search')} className="mb-2 w-full rounded-md border border-input bg-background px-3 py-2 text-sm outline-none ring-ring focus-visible:ring-2"/>
            <div className="flex-1 space-y-1 overflow-y-auto pr-1">
              {filtered.map((f) => (<button key={f} type="button" onClick={() => choose(f)} className="flex w-full items-center justify-between gap-3 rounded-md px-3 py-2 text-left text-sm hover:bg-accent">
                  <span className="truncate" style={{ fontFamily: f }}>
                    {f}
                  </span>
                  <span className="shrink-0 text-xs text-muted-foreground">Aa</span>
                </button>))}
              {filtered.length === 0 && (<p className="py-6 text-center text-sm text-muted-foreground">{t('font.empty')}</p>)}
            </div>
          </DialogContent>
        </Dialog>
      </CardHeader>
