...
function safeStyle(el: Element): React.CSSProperties | undefined {
  ...
  for (const decl of raw.split(';')) {
    ...
    if (!value) continue
    if (prop === 'color' && (COLOR_HEX.test(value) || COLOR_NAME.test(value))) style.color = value
    else if (prop === 'font-size' && /^\d+(?:\.\d+)?px$/.test(value)) style.fontSize = value
  }
...
function renderNode(node: Node, key: number): React.ReactNode {
  ...
  switch (tag) {
    ...
    case 'span':
      return (
        <span key={key} style={safeStyle(el)}>
          {kids}
        </span>
      )
    case 'p':
    ...
    case 'pre':
      return (
        <pre key={key} className="whitespace-pre-wrap">
          {kids}
        </pre>
      )
    case 'blockquote':
      return (
        <blockquote key={key} className="border-l-2 border-muted pl-2">
          {kids}
        </blockquote>
      )
    default:
...
export function RichText({
  ...
  const content = useMemo(() => {
    ...
    const doc = new DOMParser().parseFromString(
      `<div id="__rt">${normalizeRichText(text)}</div>`,
      'text/html'
    )
