...
export function FishSplash(): React.JSX.Element {
    ...
    useEffect(() => {
        if (locale === 'fish' && prev.current !== 'fish')
            setShow(true);
        prev.current = locale;
    }, [locale]);
    if (!show)
        return <></>;
    return (<div className="fixed inset-0 z-[200] flex cursor-pointer flex-col items-center justify-center overflow-hidden" onClick={() => setShow(false)} style={{ backgroundImage: `url(${fishGif})`, backgroundSize: '120px 120px' }}>
