...
export async function listSystemFonts(): Promise<string[]> {
    ...
    try {
        const out = await new Promise<string>((resolve, reject) => {
            ...
            ], { windowsHide: true, timeout: 20000, maxBuffer: 16 * 1024 * 1024 }, (err, stdout) => (err ? reject(err) : resolve(stdout)));
        });
        const found = new Set(out
            .split(/\r?\n/)
            .map((s) => s.trim())
            .filter(Boolean));
        for (const f of COMMON_CJK_FONTS)
            found.add(f);
...
export function registerAppIpc(): void {
    ...
    ipcMain.handle('app:set-font', async (_e, { font }: {
        font: string;
    }): Promise<void> => {
        setFontFamily(font);
        for (const win of BrowserWindow.getAllWindows()) {
    ...
    ipcMain.handle('app:set-locale', async (_e, { locale }: {
        locale: string;
    }): Promise<void> => {
        setLocale(locale);
        for (const win of BrowserWindow.getAllWindows()) {
