...
export function registerSetupIpc(getWebContents: () => WebContents | null): void {
    ipcMain.handle('setup:status', async (): Promise<Result<{
        hasBepInEx: boolean;
    }>> => {
        ...
        try {
            ...
            if (!pluginsReady()) {
                ...
                if (!pluginsReady()) {
                    return {
                        ok: false,
                        error: 'DevApi installation failed: the game did not generate a BepInEx plugins folder after first launch. Please launch the game once manually, then retry.'
                    };
                }
            }
