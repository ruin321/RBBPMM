...
import { bepinexPatchersDir, BEPINEX_CONFIG_FOLDER, GMP_METADATA_FOLDER, TEMP_FOLDER } from '../constants';
...
function cfgFiles(gameRoot: string, names: string[]): string[] {
    ...
    for (const n of names) {
        ...
        if (stem && !out.includes(`${stem}.cfg`))
            out.push(path.join(configDir, `${stem}.cfg`));
    }
    return out;
}
interface Backup {
...
function backupTargets(gameRoot: string, backupRoot: string, targets: string[]): Backup[] {
    ...
    for (const b of backedUp.reverse()) {
        try {
            ...
            if (fs.existsSync(b.src))
                fs.rmSync(b.src, { recursive: true, force: true });
...
export function deleteMod(gameRoot: string, installDir: string, manifest: ModManifestDto): void {
    ...
    try {
        ...
        for (const a of manifest.assets) {
            if (a.destination)
                targets.push(path.resolve(gameRoot, a.destination));
        }
...
export function deleteLegacyPlugin(gameRoot: string, installDir: string, pluginFiles: string[]): void {
    ...
    for (const pf of pluginFiles) {
        ...
        if (isInside(installDir, abs) && isInside(gameRoot, abs))
            lookups.push(abs);
    }
    const scanRoot = installDir !== pluginsDir && isInside(pluginsDir, installDir) && fs.existsSync(installDir)
        ? installDir
        : pluginsDir;
    if (fs.existsSync(scanRoot)) {
        for (const e of fs.readdirSync(scanRoot, { withFileTypes: true })) {
            ...
            if (stems.includes(stem)) {
                ...
                if (isInside(gameRoot, abs) && isInside(pluginsDir, abs))
                    lookups.push(abs);
            }
        }
    }
    ...
    try {
        ...
        for (const t of targets) {
            if (!fs.existsSync(t))
                continue;
            if (!isInside(gameRoot, t))
                throw new Error(`refusing to modify path outside root: ${t}`);
            if (fs.statSync(t).isFile())
                fs.rmSync(t, { force: true });
            else
                removeSafe(t, gameRoot);
        }
        if (installDir !== pluginsDir &&
