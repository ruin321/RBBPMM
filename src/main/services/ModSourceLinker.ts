...
function expectedNamesFor(mod: ModItemDto, manifest: ModManifestDto): string[] {
  const names: string[] = []
  const meta = loadMetadata(mod.installDir, manifest)
  if (meta.lastInstalledArchiveName) names.push(meta.lastInstalledArchiveName)
  names.push(manifest.name)
  if (mod.directoryName) names.push(mod.directoryName)
  return names
}
...
export function matchFile(
  ...
  for (const f of all) {
    ...
    if (expected.includes(fn)) {
      if (!exact || (f.dateAdded ?? 0) > (exact.dateAdded ?? 0)) exact = f
    }
  }
...
export async function linkByLocalMod(
  mod: ModItemDto,
  manifest: ModManifestDto
): Promise<{ linked: boolean; submission?: GamebananaSubmissionDto; file?: GamebananaFileDto }> {
...
export async function linkKnownSubmission(
  mod: ModItemDto,
  manifest: ModManifestDto,
  submission: GamebananaSubmissionDto,
...
export async function updateMod(
  ...
  const source = readSource(mod.installDir, manifest)
  if (!source) return { ok: false, error: 'No linked source' }

  const sub = await getSubmission(source.submissionId)
  const all = [...(sub.files ?? []), ...(sub.archivedFiles ?? [])]
  const file = all.find((f) => f.id === source.fileId) ?? all[0]
  if (!file || !file.downloadUrl) return { ok: false, error: 'No downloadable file' }
  ...
  try {
    ...
    if (fs.existsSync(dotOld)) fs.rmSync(dotOld, { recursive: true, force: true })
