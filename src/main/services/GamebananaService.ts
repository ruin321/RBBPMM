...
import type { GamebananaAlternateSourceDto, GamebananaCommentDto, GamebananaCommentsDto, GamebananaFileDto, GamebananaRequirementDto, GamebananaSearchResult, GamebananaSubmissionDto, GamebananaUpdateChangeDto, GamebananaUpdateDto, GamebananaUpdatesDto } from '../../shared/types';
...
function timedFetch(url: string, init: {
    ...
    return new Promise<Response>((resolve, reject) => {
        ...
        fetch(url, { headers: init.headers, redirect: 'follow', signal: ctrl.signal }).then((res) => {
            cleanup();
            resolve(res);
        }, (err) => {
            cleanup();
...
function timedNetFetch(url: string, init: {
    headers?: Record<string, string>;
    signal?: AbortSignal;
}, timeoutMs: number): Promise<Response> {
    return new Promise<Response>((resolve, reject) => {
        ...
        net.fetch(url, { headers: init.headers, redirect: 'follow', signal: ctrl.signal }).then((res) => {
            cleanup();
            resolve(res);
        }, (err) => {
            cleanup();
...
export async function searchMods(page: number, query?: string, category?: number): Promise<GamebananaSearchResult> {
    ...
    if (q)
        filters.push(`_aFilters[Generic_Name]=contains,${encodeURIComponent(q)}`);
    const url = `${API_BASE}Mod/Index?_nPerpage=${PAGE_SIZE}` +
...
export async function getSubmission(id: number): Promise<GamebananaSubmissionDto> {
    if (!Number.isFinite(id) || id <= 0)
...
function parseUpdate(r: unknown): GamebananaUpdateDto | null {
    if (!ok(r))
        return null;
    if (r['_bIsTrashed'] === true || r['_bIsPrivate'] === true)
        return null;
...
export async function getUpdates(submissionId: number): Promise<GamebananaUpdatesDto> {
    ...
    try {
        const doc = await getJson(`${API_BASE}Mod/${submissionId}/Updates?_nPerpage=${PAGE_SIZE}&_nPage=1`);
        const records = doc['_aRecords'];
        const meta = ok(doc['_aMetadata']) ? doc['_aMetadata'] : undefined;
...
export async function downloadMod(downloadUrl: string, onProgress?: (p: DownloadProgress) => void, isCancelled?: () => boolean): Promise<string> {
