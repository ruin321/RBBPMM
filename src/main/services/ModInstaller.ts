...
function relatedFiles(dir: string, base: string): string[] {
  ...
  for (const f of fs.existsSync(dir) ? fs.readdirSync(dir) : []) {
    if (f === `${base}.dll` || f === `${base}.xml` || f === `${base}.pdb`) out.push(f)
  }
  return out
}
...
export async function installModArchive(
  ...
  isCancelled?: () => boolean,
  preExtractedRoot?: string
): Promise<{ mod: ModItemDto; warnings: SecurityWarning[] }> {
  ...
  const rollback = (): void => {
    for (const m of moved.reverse()) {
      try {
        if (fs.existsSync(m.to)) fs.rmSync(m.to, { recursive: true, force: true })
      } catch {
  ...
  try {
    ...
    if (!parsed) throw new Error('No valid manifest.json found in archive')
    manifest = parsed
    ...
    for (const p of manifest.plugins) {
      ...
      if (!isInside(extractRoot, src)) throw new Error(`Plugin escapes temp: ${p}`)
    ...
    for (const p of manifest.patchers) {
      ...
      if (!isInside(extractRoot, src)) throw new Error(`Patcher escapes temp: ${p}`)
      if (!fs.existsSync(src)) continue
    ...
    for (let i = 0; i < manifest.assets.length; i++) {
      ...
      if (!isInside(gameRoot, dest)) throw new Error(`Asset destination escapes game root: ${dest}`)
      const src = path.resolve(extractRoot, normalizeRel(asset.localPath))
      if (!isInside(extractRoot, src)) continue
      if (!fs.existsSync(src)) continue
    ...
    if (ownsTemp) {
      try {
        if (fs.existsSync(gameRoot) && isInside(gameRoot, tempRoot)) removeDirIfInside(gameRoot, tempRoot)
      } catch {
...
export function hasManifest(extractRoot: string): boolean {
  for (const f of [GMP_METADATA_FOLDER, GMP_FALLBACK_METADATA_FOLDER]) {
    if (fs.existsSync(path.join(extractRoot, f, 'manifest.json'))) return true
  }
  return false
}
...
function copyDirContents(srcDir: string, destDir: string, moved: { from: string; to: string }[]): void {
  ...
  for (const f of [GMP_METADATA_FOLDER, GMP_FALLBACK_METADATA_FOLDER]) {
    ...
    if (fs.existsSync(p)) {
      try {
        ...
        if (!m?.guid || !m?.name || !m?.author || m?.version === undefined) continue
        m.plugins = m.plugins || []
...
function copyTreeContents(srcDir: string, destDir: string): void {
  ...
  for (const name of fs.readdirSync(srcDir)) {
    const from = path.join(srcDir, name)
    const to = path.join(destDir, name)
    if (fs.statSync(from).isDirectory()) copyTreeContents(from, to)
...
export function installUnmanaged(
  extractRoot: string,
  gameRoot: string,
  onProgress?: ProgressCb,
  isCancelled?: () => boolean
  ...
  try {
    ...
    for (const item of targets.modded) {
      ...
      if (!isInside(moddedRoot, dest)) throw new Error(`Modded dest escapes: ${dest}`)
    ...
    for (const plugin of targets.plugins) {
      ...
      if (!isInside(pluginsRoot, dest)) throw new Error(`plugin dest escapes: ${dest}`)
      if (!isInside(gameRoot, dest)) throw new Error(`dest outside game root: ${dest}`)
