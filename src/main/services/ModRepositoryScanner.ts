...
function dirInstalledAt(abs: string): number | undefined {
    try {
        ...
        if (Number.isFinite(st.mtimeMs) && st.mtimeMs > 0)
...
function moddedFolderMap(gameRoot: string): Map<string, string> | undefined {
    ...
    try {
        for (const e of fs.readdirSync(root, { withFileTypes: true })) {
            if (e.isDirectory())
                map.set(e.name.toLowerCase(), path.join(root, e.name));
        }
    }
    catch {
    }
...
function titleMatchScore(title: string | undefined, guid: string | undefined): number {
    ...
    if (t === last || t === g)
        return 3;
    if (last.includes(t) || t.includes(last))
        return 2;
    const tJ = t.replace(/[^a-z0-9]/g, '');
    const gJ = g.replace(/[^a-z0-9]/g, '');
    if (gJ.includes(tJ) || tJ.includes(gJ))
        return 1;
    return 0;
}
export function buildModItem(gameRoot: string, modRoot: string, manifest: ModManifestDto, supports: boolean): ModItemDto {
...
function parsePluginEntry(name: string): {
    stem: string;
    file: string;
    disabled: boolean;
} | null {
    ...
    if (lc.endsWith('.dll'))
        return { stem: name.slice(0, -4), file: name, disabled: false };
    const backup = /^(.*\.dll)\.\d+$/i.exec(name);
    if (backup)
        return { stem: backup[1].slice(0, -4), file: backup[1], disabled: true };
    return null;
}
function collectPlugins(dir: string, prefix = '', out: {
    file: string;
    disabled: boolean;
}[] = []): {
    file: string;
    ...
    for (const f of fs.readdirSync(dir, { withFileTypes: true })) {
        ...
        if (f.isFile()) {
            ...
            if (norm)
                out.push({ file: norm.file === f.name ? rel : path.join(prefix, norm.file), disabled: norm.disabled });
        }
...
function buildLegacyItem(installDir: string, title: string, plugins: string[], activated: boolean, identifyName?: string, group?: string, guidOverride?: string): ModItemDto {
    return {
...
const SCAN_CACHE = new Map<string, {
    gameVersion?: string;
    mods: ModItemDto[];
}>();
...
export function patchModScanEntry(gameRoot: string, guid: string, apply: (mod: ModItemDto) => void): boolean {
    const prefix = String(gameRoot).toLowerCase();
    let hit = false;
...
export function scanRepository(gameRoot: string, gameVersion?: string): ModItemDto[] {
    ...
    for (const entry of entries) {
        ...
        if (manifest) {
            ...
            for (const p of manifest.plugins) {
                ...
                if (norm)
                    coveredStems.add(norm.stem.toLowerCase());
            }
            continue;
        }
        ...
        for (const p of plugins) {
            ...
            if (p.disabled)
                cur.disabled = cur.disabled ?? p.file;
            else
                cur.active = cur.active ?? p.file;
            byStem.set(key, cur);
        }
        for (const { stem, active, disabled } of byStem.values()) {
            ...
            const item = buildLegacyItem(modRoot, stem, [file], activated, resolveIdentifyName(entry.name, candidates), entry.name, `legacy:${entry.name}/${stem}`);
            mods.push(item);
    ...
    const rootIndex = new Map<string, {
        title: string;
        active: string[];
        disabled: string[];
    }>();
    for (const entry of entries) {
        ...
        if (norm.disabled)
            group.disabled.push(norm.file);
        else
            group.active.push(norm.file);
    }
    for (const g of roots) {
        if (g.active.length > 0) {
            const candidates = collectCandidates(pluginsDir, g.active);
            const item = buildLegacyItem(pluginsDir, g.title, g.active, true, resolveIdentifyName(g.title, candidates));
            mods.push(item);
            legacyMetas.push({ item, candidates });
        }
        else if (g.disabled.length > 0) {
            const candidates = collectCandidates(pluginsDir, [g.disabled[0]]);
            const item = buildLegacyItem(pluginsDir, g.title, [g.disabled[0]], false, resolveIdentifyName(g.title, candidates));
            mods.push(item);
            legacyMetas.push({ item, candidates });
        }
    }
    ...
    if (dirs && legacyMetas.length > 0) {
        ...
        for (const m of mods) {
            if (m.moddedFolder)
                claimed.add(path.basename(m.moddedFolder).toLowerCase());
        }
        for (const le of legacyMetas) {
        ...
        const identOwners = new Map<string, {
            le: {
                item: ModItemDto;
            };
            score: number;
        ...
        for (const le of legacyMetas) {
            ...
            if (!le.item.moddedFolder) {
                ...
                if (tf && !claimed.has(path.basename(tf).toLowerCase()))
...
export function applyResolvedPaths(gameRoot: string, m: ModItemDto): void {
    const pluginsDir = bepinexPluginsDir(gameRoot);
    const active = m.pluginFiles.find((p) => /\.dll$/i.test(p) && !/\.(disabled|disable|\.\d+)$/i.test(p)) ??
        m.pluginFiles[0];
    if (active) {
...
function findConfigFile(configDir: string, m: ModItemDto): string | undefined {
    let cfgFiles: string[];
    try {
    ...
    for (const stem of stems) {
        ...
        if (hit)
            return path.join(configDir, hit);
    }
    ...
    if (idn) {
        ...
        if (byGuid)
            return path.join(configDir, byGuid);
    }
    return undefined;
}
