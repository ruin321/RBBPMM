...
function resolve7za(): string {
    ...
    if (fs.existsSync(bundled7z))
        candidates.push(bundled7z);
    const localAppData = process.env['LOCALAPPDATA'] || '';
    const programFiles = process.env['ProgramFiles'] || '';
    const programFilesX86 = process.env['ProgramFiles(x86)'] || '';
    candidates.push(path.join(localAppData, 'Microsoft', 'WindowsApps', '7z.exe'), path.join(programFiles, '7-Zip', '7z.exe'), path.join(programFilesX86, '7-Zip', '7z.exe'));
    candidates.push(path7za);
    candidates.push(path.join(app.getAppPath(), 'node_modules', '7zip-bin', 'win', sevenZipArch(), '7za.exe'));
        ...
        await new Promise<void>((resolve, reject) => {
            ...
            child.on('close', (code) => {
                if (code === 0)
                    ...
                    reject(new Error(`Extract failed (7za exit ${code}): ${archivePath}`));
            });
        });
    }
