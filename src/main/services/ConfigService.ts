...
function controlFor(rawType: string | undefined, value: string, accept?: string[]): {
    control: CfgEntryDto['control'];
    step?: number;
} {
...
export function parseCfgFile(cfgPath: string): ConfigFileDto | undefined {
    ...
    for (const raw of rawLines) {
        ...
        if (mType || mDef || mAcc || mRange || mMin || mMax) {
            pending ??= { desc: [] };
            if (mType)
                pending.type = mType[1].trim();
            if (mDef)
                pending.def = mDef[1].trim();
            if (mAcc)
                pending.accept = mAcc[1].split(',').map((s) => s.trim()).filter(Boolean);
            if (mRange) {
                pending.min = Number(mRange[1]);
                pending.max = Number(mRange[2]);
            }
            if (mMin)
                pending.min = Number(mMin[1]);
            if (mMax)
                pending.max = Number(mMax[1]);
            continue;
        }
        if (t.startsWith('#')) {
            let content = t;
