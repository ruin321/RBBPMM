...
export function detectBepInEx(gameRoot: string): boolean {
    ...
    if (fs.existsSync(path.join(root, 'config', 'BepInEx.cfg')))
        return true;
    if (fs.existsSync(path.join(root, 'core', 'BepInEx.Core.dll')))
        return true;
...
export async function installBepInEx(gameRoot: string, onProgress?: (p: InstallProgress) => void): Promise<Result<boolean>> {
